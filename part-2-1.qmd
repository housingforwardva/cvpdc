...
```{r}
#| label: fig-multigen
#| fig-cap: "Percent of population in multigenerational households"
...
multigen_plot <- ggplot(multigen_data,
  ...
  scale_y_continuous(labels = label_percent(),
                     breaks = c((1:9)/100),
                     limits = c(0,0.09)) +
  labs(title = "Percent of population in multigenerational households",
...
```{r}
#| label: fig-mhc-map
#| fig-cap: "Manufactured home communities in Lynchburg region"
...
if (knitr::is_html_output()) {
  ...
  ggplot() +
    ...
    labs(title = "Manufactured home communities in Lynchburg region",
         color = "Units in park",
         caption = "**Source:** U.S. Department of Homeland Security. Homeland Infrastructure Foundation-Level Data (HIFLD).") +
    theme_hfv() +
    theme(
      #plot.title = element_markdown(margin = margin(b = 10, unit = "pt")),
...
ggplot(cv_mort,
  ...
  scale_y_continuous(labels = number_format(big.mark = ",")) +
  labs(
    title = "Loans by occupancy type",
    subtitle = "Single-family home loans originated in the region",
    caption = "**Source:** Consumer Protection Financial Bureau, Home Mortgage Disclosure Act (HMDA) Snapshot National Loan Level Dataset."
    )
...
hcv_grp_plot <- ggplot(hcv_group,
       aes(x = reorder(county_name, estimate, decreasing = TRUE),
           y = estimate,
           fill = county_name)) +
  geom_col() +
  geom_col_interactive() +
...
if (knitr::is_html_output()) {
  ...
  ggplot() +
    cv_basemap +
    layer_spatial(hcv, aes(fill = hcv_public_pct), alpha = 0.9, color = "grey70") +
    ...
    theme_hfv() +
    labs(title = "Percent of renters with Housing Choice Vouchers by tract",
         fill = "Percent",
...
cbhh_region_plot <- ggplot(cbhh_region,
  ...
  labs(title = "Cost burden by household income",
       subtitle = "Percent of households with cost burden by AMI from 2012 to 2020",
       caption = "**Source:** U.S Department of Housing and Urban Development, Comprehensive Housing Affordability Strategy (CHAS), Table 7.") +
  add_zero_line("y") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5))
