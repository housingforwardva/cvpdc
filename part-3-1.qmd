...
cbhh_region <- cb_7 %>% 
  group_by(year, tenure, household_income, cb_group) %>% 
  summarise(estimate = sum(estimate)) %>% 
  group_by(year, tenure) %>% 
  mutate(pct = estimate/sum(estimate)) 
...
cbhh_region_plot <- ggplot(cbhh_region,
       aes(x = household_income, 
           ...
           data_id = run_diff,
           tooltip = label_number(big.mark = ",")(run_diff))) +
  geom_col(position = "dodge") +
  geom_col_interactive(position = "dodge", size = 2) +
  theme_hfv() +
  scale_fill_hfv() +
  ...
  scale_y_continuous(labels = label_comma()) +
  labs(title = "Cumulative change in cost-burdened households by income",
