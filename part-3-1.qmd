...
# Get population estimates from the Population Estimates Program (PEP) from 2010 to 2019.
...
years <- 2014:2020 # Not including pre-2014 to avoid Bedford city. Cannot aggregate medians.
...
mhc <- st_read("https://services1.arcgis.com/Hp6G80Pky0om7QvQ/arcgis/rest/services/Mobile_Home_Parks/FeatureServer/0/query?outFields=*&where=1%3D1&f=geojson", quiet = TRUE) |> 
  filter(STATE == "VA") %>% 
  filter(str_detect(COUNTY, "AMHERST|APPOMATTOX|CAMPBELL|LYNCHBURG"))
...
cbps_ytd <- read_csv("https://www2.census.gov/econ/bps/County/co2209y.txt", 
                     col_names = FALSE,
                     skip = 2) |> 
  select(X1:X18) |> 
  set_names(column_names) |> 
...
write_rds(rr_cbps, "data/cv_cbps.rds")
```
