...
fred <- FredR(api.key = Sys.getenv("api.key"))

cpi_ls <- fred$series.observations(series_id = 'CUUR0000SA0L2') %>% 
  select(date, value) %>% 
  mutate(date = as.Date(date)) %>% 
...
cv_sold_plot <- ggplot(cv_sold,
  ...
  labs(title = "Number of closed sales",
       caption = "**Source:** Lynchburg REALTORS Multiple Listing Service (MLS)")
...
cv_dom_plot <- ggplot(cv_dom,
  ...
  theme_hfv() + 
  scale_fill_gradientn(colours = c("#ADDCB1", "#46A88F", "#207A88", "#0F1A4E")) +
  labs(title = "Average days on market",
       caption = "**Source:** Lynchburg REALTORS Multiple Listing Service (MLS)")
