...
From 2010 to 2020, the Lynchburg region only grew by six percent, an increase of just over 15,000 people. This slow but steady growth across the last decade was punctuated by a slight decrease in population between 2019 and 2020. 
...
```{r}
#| label: fig-pchange
#| fig-cap: "Regional components of population change"
...
```
Despite the slight population decline from from 2019 to 2020, the region is projected to near 300,000 people by 2050 --- a 13 percent increase from the 2020 Census.
```{r}
...
```{r}
#| label: fig-tenure
#| fig-cap: "Regional change in household tenure"
...
```{r}
#| label: fig-age
#| fig-cap: "Regional change in households by age and tenure"
...
```{r}
#| label: fig-hhtype
#| fig-cap: "Regional change in households by type"
...
```{r}
#| label: fig-hhsize
#| fig-cap: "Regional change in households by size and tenure"
...
```{r}
#| label: fig-seniors
#| fig-cap: "Regional change in senior population"
...
```
With housing affordability becoming more and more of a challenge for families, some families are forced to live with others. These families are called subfamilies, and within the Lynchburg region subfamilies are more than likely to be single mothers. But in recent years, the number of single fathers and married couples (both with and without children) living with another family has been on the rise.
```{r}
...
```{r}
#| label: fig-subfam
#| fig-cap: "Regional change in families living with others"
...
```{r}
#| label: fig-multigen
#| fig-cap: "Regional percentage of multigenerational households"
...
#| fig-cap: "Regional change in young adult population by living arrangement"
...
```{r}
#| label: fig-incdist
#| fig-cap: "Regional income distribution by tenure"
...
```{r}
#| label: fig-medinc
#| fig-cap: "Regional median household income by tenure"
...
#| fig-cap: "Regional median household income by race and ethnicity"
...
```{r}
#| label: fig-structure
#| fig-cap: "Regional housing stock by structure type"
...
```{r}
#| label: fig-yrbuilt
#| fig-cap: "Regional housing stock by year built"
...
```{r}
#| label: fig-beds
#| fig-cap: "Change in regional housing stock by bedroom count"
...
```{r}
#| label: fig-permits
#| fig-cap: "Regional residential building permits"
...
```{r}
#| label: fig-ho-rate
#| fig-cap: "Regional homeownership rate"
...
cv_ho_plot <- ggplot(cv_ho,
  ...
  labs(title = "Regional homeownership rate",
       caption = "**Source:** U.S. Census Bureau, ACS 5-Year Estimates, Table B25003.")
...
```{r}
#| label: fig-mls-price
#| fig-cap: "Regional median residential sales price"
...
cv_price_adj <- cv_price |> 
  ...
  mutate(type = case_when(
    ...
    type == "price_adj" ~ "Inflation-adjusted dollars"
  )) |> 
...
cv_price_plot <- ggplot(cv_price_adj,
  ...
  labs(title = "Median residential sales price",
...
```{r}
#| label: fig-mls-sales
#| fig-cap: "Regional number of closed home sales"
...
cv_sold_plot <- ggplot(cv_sold,
  ...
  labs(title = "Number of closed home sales",
       caption = "**Source:** Lynchburg REALTORS Multiple Listing Service (MLS)")

if (knitr::is_html_output()) {
  
  girafe(ggobj = cv_sold_plot,
         height_svg = 4)
...
```{r}
#| label: fig-mls-dom
#| fig-cap: "Regional average days on market"
...
cv_dom_plot <- ggplot(cv_dom,
  ...
  theme_hfv() + 
  scale_fill_gradientn(colours = c("#ADDCB1", "#46A88F", "#207A88", "#0F1A4E")) +
  labs(title = "Average days on market for residential",
       caption = "**Source:** Lynchburg REALTORS Multiple Listing Service (MLS)")

if (knitr::is_html_output()) {
  
  girafe(ggobj = cv_dom_plot,
         height_svg = 4)
...
```{r}
#| label: fig-rent
#| fig-cap: "Regional average market asking rent"
...
cvpdc_rent <- read_csv("data/cvpdc_rent.csv") |>
  ...
  mutate(value = case_when(
    ...
    value == "infl_adj" ~ "Inflation-adjusted dollars"
  )) |> 
...
```{r}
#| label: fig-vacancy
#| fig-cap: "Regional rental vacancy rate"
...
if (knitr::is_html_output()) {
  ...
  girafe(ggobj = cvpdc_vacancy_plot,
         height_svg = 4)
...
As home prices increase and rental supply wanes, the impacts on housing affordability can
...
```{r}
#| label: fig-cb-total
#| fig-cap: "Total regional housing cost burden by household area median income"
...
cbhh_region_plot <- ggplot(cbhh_region,
  ...
  scale_y_continuous(labels = percent_format()) +
  labs(title = "Cost-burdened households by household area median income",
...
```{r}
#| label: fig-cb-change
#| fig-cap: "Regional change in cost-burdened households by tenure"
...
cb_9 <- read_csv("data/Table9_2012to2019.csv") |> 
  ...
  select(county, fips, year, race, cost_burden, cb_group, estimate)
...
```{r}
#| label: fig-cb-race
#| fig-cap: "Regional percent of cost-burdened households by race and ethnicity"
...
```{r}
#| label: fig-hh-cb
#| fig-cap: "Regional cost-burden by household type"
