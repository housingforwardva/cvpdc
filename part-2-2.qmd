# County market analyses {#part-2-2}
...
# Add LEHD data -- commute inflow/outflow by locality?
...
# Add single-family rent estimates from ACS for MSA - not sure what table this is? Would you take aggregate gross rents by units in structure and then divide by the total number of structures in those categories to get the average???
...
local_pop <- pop_data |> 
  ...
  # filter(year == 2020) |> 
  select(GEOID, year, county, value, pchg)
...
local_growth_plot <- ggplot(local_pop,
  ...
  labs(title = "Population growth by county",
       subtitle = "Percent change in population from 2010 to 2022",
...
subtitle_text <- "Among <span style = 'color:#011E41'><b>homeowners</b></span> and <span style = 'color:#40C0C0'><b>renters</b></span> from 2010 to 2021"
...
local_age_plot <- ggplot(local_age,
  ...
  labs(title = "Percent change in households by age",
       subtitle = subtitle_text,
...
cv_ad <- b18105_data |> 
  group_by(year, NAME, age_grp, ad) |> 
  summarise(estimate = sum(estimate)) |> 
  filter(ad == "With an ambulatory difficulty") |> 
  mutate(year = as.character(year))

subtitle_text <- "Adults <span style = 'color:#011E41'><b>65 years and over</b></span> and <span style = 'color:#40C0C0'><b>under 65 years old</b></span>"
...
subtitle_text <- "For <span style = 'color:#011E41'><b>homeowners</b></span> and <span style = 'color:#40C0C0'><b>renters</b></span> in 2021"
...
local_dist_plot <- ggplot(local_dist,
  ...
  scale_y_continuous(labels = label_comma()) +
  labs(title = "Households by income",
       subtitle = subtitle_text,
...
Insert ttext about avg wages in the county.

:::
```{r}
#| label: fig-qcew
#| fig-cap: "Average annual wage"

cv_qcew <- read_rds("data/qcew_data.rds") |> 
  filter(area_fips != 51680) |> 
  mutate(GEOID = as.character(area_fips)) |> 
  left_join(cv_names, by = "GEOID") |> 
  select(county, year, adj_pay)

subtitle_text <- "In <span style = 'color:#011E41'><b>Amherst</b></span>, <span style = 'color:#40C0C0'><b>Appomattox</b></span>, <span style = 'color:#8B85CA'><b>Bedford</b></span>, and <span style = 'color:#259591'><b>Campbell</b></span>"

qcew_plot <- ggplot(cv_qcew,
       aes(x = year,
           y = adj_pay,
           color = county)) +
  geom_line() +
  geom_point_interactive(
    aes(data_id = adj_pay,
        tooltip = dollar_format()(adj_pay)),
    size = 2) + 
  theme_hfv() +
  scale_color_hfv() +
  labs(title = "Average annual wage",
       subtitle = subtitle_text,
       caption = "**Source:** U.S. Bureau of Labor Statistics, Quarterly Census of Employment and Wages (QCEW).") +
...
cv_cbps_plot <- ggplot(cv_cbps,
  ...
  labs(title = "New residential building permits by type",
       subtitle = "Annual permits issued by locality from 2000 through April 2023",
...
rs_vacancy <- read_rds("data/b25004_data.rds") |> 
  ...
  group_by(year, NAME, tenure, status) |> 
  summarise(estimate = sum(estimate))
...
ggplot(rs_vacancy,
       aes(x = year,
           ...
           fill = tenure,
           data_id = estimate,
        tooltip = number_format()(estimate))) +
    geom_col(position = "stack") +
     geom_col_interactive(position = "stack") +
  facet_wrap(~NAME, nrow = 1) +
  ...
  theme_hfv() +
  scale_fill_hfv() +
  labs(title = "Vacancy among rental and homeowner stock",
...
ostatus <- read_rds("data/b25004_data.rds") |> 
  filter(NAME != "Lynchburg city") |> 
  filter(status == "For seasonal, recreational, or occasional use" |
           status == "Other vacant") |> 
  group_by(year, NAME, status) |> 
  summarise(estimate = sum(estimate)) |> 
  mutate(year = as.character(year))
...
ggplot(ostatus,
       aes(x = year,
           y = estimate,
           fill = str_wrap(status, 15),
           data_id = estimate,
           tooltip = number_format(big.mark = ",")(estimate))) +
  geom_col(position = "stack") + 
  geom_col_interactive(position = "stack") +
  facet_wrap(~NAME, labeller = label_wrap_gen(15)) + 
  theme_hfv() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        legend.position = "right") +
  scale_fill_hfv() +
...
#| fig-cap: "Median quarterly sales price"
...
sales <- read_rds("data/cv_var.rds") |> 
  filter(name != "Lynchburg City") |> 
  select(geography, name, med_price, adj_price, period, quarter, year) |> 
  mutate(period = as.character(period)) |> 
  select(geography, name, adj_price, quarter) |> 
  filter(quarter == "2023 Q2" | quarter == "2019 Q2") 
...
ggplot(sales, 
  ...
  geom_point_interactive(
    ...
    size = 2) + 
  geom_text(aes(label = dollar(adj_price), hjust = ifelse(quarter == "2023 Q2", -0.25, 1.25))) +
  coord_flip() +
  theme_hfv() +
  scale_y_continuous(labels = dollar_format(), limits = c(100000, 450000)) +
  labs(title = "Median quarterly residential sales price",
       subtitle = subtitle_text,
...
ggplot(units, 
  ...
  labs(title = "Total homes sold by county",
       caption = "**Source:** Virginia REALTORS, through Q2 2023.")
...
ggplot(dom, 
  ...
  scale_fill_hfv() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Median days on market by county",
       caption = "**Source:** Virginia REALTORS, through Q2 2023.") 
...
# Note the download date of data here: 9/21/23
...
```
Rental vacancy rates among multifamily housing is lowest among Amherst and Appomattox properties. Bedford and Campbell properties have seen greater fluctuations in their
...
amherst_v <- read_csv("data/amherst_vacancy.csv") |> 
  mutate(county = "Amherst County")

appomattox_v <- read_csv("data/appomattox_vacancy.csv") |> 
  mutate(county = "Appomattox County")

bedford_v <- read_csv("data/bedford_vacancy.csv") |> 
  mutate(county = "Bedford County")

campbell_v <- read_csv("data/campbell_vacancy.csv") |> 
  mutate(county = "Campbell County")

library(zoo)
library(lubridate)

local_vacancy <- rbind(amherst_v, appomattox_v, bedford_v, campbell_v) |> 
  filter(!str_detect(period, "QTD")) |> 
  mutate(period = as.Date(as.yearqtr(period, format = "%Y Q%q"), frac = 1)) |> 
  mutate(year = year(period)) |> 
  filter(year >= 2016) |> 
...
vacancy_plot <- ggplot(local_vacancy,
  ...
  labs(title = "Rental vacancy rate by quarter",
       subtitle = "All multifamily properties in CoStar",
       caption = "**Source:** CoStar Group, Inc.")
...
Cost burden by race and ethnicity shows that white households across all four counties are less likely to be cost-burdened. This has been consistent since 2012. Bklack
...
cv_cb9 <- cb_9 |> 
  ...
  summarise(estimate = sum(estimate),
            moe = sum(moe)) |> # MOE are very high for Another race, including multiracial and Asian and Hispanic
  group_by(year, county, race) |> 
  mutate(pct = estimate/sum(estimate)) |> 
  filter(cb_group == "Cost-burdened") |> 
  filter(year == 2020) |> 
  filter(race != "Hispanic or Latino" & race != "Another race, including multiracial" & race != "Asian")

subtitle_text <- "Percent of <span style = 'color:#011E41'><b>Black</b></span> and <span style = 'color:#40C0C0'><b>white</b></span> cost-burdened households in 2020"
...
cv_cb9_plot <- ggplot(cv_cb9,
       aes(x = reorder_within(county, pct, race),  
           ...
           data_id = pct,
           tooltip = label_percent()(pct))) +
  facet_wrap(~county, nrow = 1, scales = "free_x", labeller = label_wrap_gen(width = 10)) +
  geom_col() +
  geom_col_interactive(size = 1) +
  theme_hfv() +
  add_zero_line("y") +
...
cbhh_region_plot <- ggplot(cbhh_region,
  ...
  labs(title = "Local cost burden by household area median income",
       subtitle = "Percent of cost-burdened households by household income in 2020",
...
cb_hh_plot <- ggplot(cbhh_region,
  ...
  labs(title = "Cost burden by household type",
       subtitle = "Percent of households with cost burden by household type in 2020",
...
gap_plot <- ggplot(gap,
  ...
  labs(title = "Rental housing gap by household income",
       subtitle = "Affordability of existing rental homes by AMI of current occupants (2020)",
