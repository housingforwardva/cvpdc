...
lb_basemap <- read_rds("data/lb_basemap.rds")
...
subtitle_1 <- "<span style = 'color:#011E41'><b>Census counts</b></span> and <span style = 'color:#40C0C0'><b>population estimates</b></span>"
...
subtitle_2 <- "<span style = 'color:#011E41'><b>Census counts</b></span> and <span style = 'color:#40C0C0'><b>population projections</b></span>"
...
subtitle_3 <- "<span style = 'color:#011E41'><b>Homeowners</b></span> and <span style = 'color:#40C0C0'><b>renters</b></span> from 2010 to 2021"
...
```{r}
#| label: fig-size
#| fig-cap: "Cumulative change in households by size and tenure in Lynchburg"
...
subtitle_4 <- "Change among <span style = 'color:#011E41'><b>married couples</b></span> and <span style = 'color:#40C0C0'><b>single parents</b></span> from 2010 to 2021."
...
subtitle_5 <- "For <span style = 'color:#011E41'><b>homeowners</b></span> and <span style = 'color:#40C0C0'><b>renters</b></span> in 2010 and 2021"
...
subtitle_6 <- "For <span style = 'color:#011E41'><b>homeowners</b></span> and <span style = 'color:#40C0C0'><b>renters</b></span> in 2021 dollars"
...
```{r}
#| label: fig-structure
#| fig-cap: "Lynchburg's share of regional housing stock"
...
local_str_plot <- ggplot(local_str,
  ...
  labs(title = "Lynchburg's share of regional housing stock",
      subtitle = "Percent of region's occupied homes within Lynchburg by tenure and type in 2021",
...
if (knitr::is_html_output()) {
  ...
  ggplot() +
    lb_basemap +
    geom_sf(mhc, mapping = aes(color = SIZE), size = 3, show.legend = "point") +
    scale_color_hfv() +
    scale_x_continuous(expand = expansion(mult = 0)) +
    scale_y_continuous(expand = expansion(mult = 0)) +
    labs(title = "Manufactured home communities in Lynchburg",
         color = "Units in park",
...
#| fig-cap: "Median home sales price in Lynchburg"
...
lburg_sales_plot <- ggplot(sales,
  ...
  #      panel.grid.major.x = element_line(color = "#dadada", size = 0.1)) +
  labs(title = "Median home sales price in Lynchburg",
       caption = "**Source:** Virginia REALTORS, adjusted to Q2 2023 dollars.")

publish_plot(lburg_sales_plot)
...
#| fig-cap: "Closed home sales in Lynchburg"
...
lburg_sold_plot <- ggplot(units_sold,
  ...
  labs(title = "Closed home sales in Lynchburg",
...
```{r}
#| label: fig-dom
#| fig-cap: "Median days on market in Lynchburg"
...
lburg_dom_plot <- ggplot(lburg_dom,
  ...
  add_zero_line("y") +
  scale_fill_gradientn(colours = c("#ADDCB1", "#46A88F", "#207A88", "#0F1A4E")) +
  labs(title = "Median days on market in Lynchburg",
       caption = "**Source:** Virginia REALTORS.")
...
lb_hex_count <- filter(hex_grid_sold, n_pts > 5)

write_rds(lb_hex_count, "data/lb_hex_count.rds")

```


```{r}
#| label: fig-lburg-sales-map
#| fig-cap: "Map of home sales in Lynchburg from 2016 through 2022"

lb_hex_count <- read_rds("data/lb_hex_count.rds")

lb_hex_basemap <- read_rds("data/lb_hex_basemap.rds")

if (knitr::is_html_output()) {
  
  pal <- colorRampPalette(c("#b2e5e5", "#f57dbb", "#041c3b"))
  
  mapview(
    lb_hex_count,
    zcol = "n_pts",
    layer.name = "Number of sales",
    col.regions = pal,
    alpha = 0
    )
  ...
  ggplot() +
    lb_hex_basemap +
    geom_sf(lb_hex_count, mapping = aes(fill = n_pts), alpha = 0.5, color = NA, show.legend = "polygon") +
    scale_fill_viridis_c(direction = -1) +
    scale_x_continuous(expand = expansion(mult = 0)) +
    scale_y_continuous(expand = expansion(mult = 0)) +
    labs(title = "Map of home sales in Lynchburg",
         subtitle = "2016 through 2022",
...
```{r}
#| label: fig-vacancy
#| fig-cap: "Rental vacancy in Lynchburg"
...
vacancy_plot <- ggplot(local_vacancy,
  ...
  theme_hfv() +
  scale_color_gradientn(colours = c("#ADDCB1", "#46A88F", "#207A88", "#0F1A4E")) +
  scale_y_continuous(limits = c(0,0.10), breaks = seq(0, 0.11, 0.02), labels = percent_format()) +
...
```{r}
#| label: fig-sfr-ward
#| fig-cap: "Single-family detached rentals by ward in Lynchburg"
...
sfr_ward_plot <- ggplot(sfr_ward,
  ...
  scale_fill_hfv() +
  add_zero_line("y") +
  labs(title = "Single-family detached rentals by ward in Lynchburg",
       subtitle = "Estimated via property ownership records",
       caption = "**Source:** City of Lynchburg parcel data.") 
...
```{r}
#| label: fig-lihtc-map
#| fig-cap: "Map of LIHTC properties in Lynchburg"
...
pal_lihtc <- colorFactor(palette = "viridis", levels = c(2030, 2035, 2040, 2045, 2050))
...
leaflet(lihtc_map) |> 
  ...
  addCircleMarkers(weight = 1,
                   fillOpacity = 0.6,
                   color = ~pal_lihtc(yr_group),
                   popup = paste0("Property: ", lihtc_map$property_name,
                                  "<br>", "Units: ", lihtc_map$assisted_units)) |> 
  addLegend("topright", pal = pal_lihtc, values = c(2030, 2035, 2040, 2045, 2050),
  ...
  ggplot() +
    lb_basemap +
    geom_sf(lihtc_map, mapping = aes(color = yr_group), size = 3, show.legend = "point") +
    scale_color_viridis_c(direction = -1) +
    scale_x_continuous(expand = expansion(mult = 0)) +
    scale_y_continuous(expand = expansion(mult = 0)) +
    labs(title = "Map of LIHTC properties in Lynchburg",
         subtitle = "By affordability commitment expiration date",
         color = "Expired by\nor before",
...
```{r}
#| label: fig-s8-map
#| fig-cap: "Map of Section 8 properties in Lynchburg"
...
pal_s8 <- colorFactor(palette = "viridis", levels = c(2030, 2035, 2040, 2045, 2050))
...
leaflet(s8_map) |> 
  ...
  addCircleMarkers(weight = 1,
                   fillOpacity = 0.6,
                   color = ~pal_s8(yr_group),
                   popup = paste0("Property: ", s8_map$property_name,
                                  "<br>", "Units: ", s8_map$assisted_units)) |>
  addLegend("topright", pal = pal_s8, values = c(2030, 2035, 2040, 2045, 2050),
            title = "Expired by <br>or before")
  ...
  ggplot() +
    lb_basemap +
    geom_sf(s8_map, mapping = aes(color = yr_group), size = 3, show.legend = "point") +
    scale_color_viridis_c(direction = -1) +
    scale_x_continuous(expand = expansion(mult = 0)) +
    scale_y_continuous(expand = expansion(mult = 0)) +
    labs(title = "Map of Section 8 properties in Lynchburg",
         subtitle = "By affordability commitment expiration date",
         color = "Expired by\nor before",
         caption = "**Source:** National Housing Preservation Database.") +
    theme_hfv() +
...
```{r}
#| label: fig-hcv-map
#| fig-cap: "Map of Housing Choice Voucher utilization in Lynchburg by tract"
...
pal_hcv <- colorNumeric(palette = "viridis", domain = hcv$hcv_pct_renter_occ_units, na.color = "transparent", reverse = TRUE)
...
leaflet(hcv) |>
  ...
  addPolygons(color = "darkgrey",
              weight = 0.6,
              fillColor = ~pal_hcv(hcv_pct_renter_occ_units),
              fillOpacity = 0.6,
              popup = ~label) |> 
  addLegend("topright", pal = pal_hcv,
            values = ~hcv_pct_renter_occ_units, labFormat = labelFormat(suffix = "%"),
            title = "Percent of renters <br> utilizing HCV"
            )
  ...
  ggplot() +
    lb_basemap +
    geom_sf(hcv, mapping = aes(fill = hcv_pct_renter_occ_units), alpha = 0.9, color = "grey70") +
    ...
    scale_x_continuous(expand = expansion(mult = 0)) +
    scale_y_continuous(expand = expansion(mult = 0)) +
    labs(title = "Map of HCV utilization in Lynchburg",
         fill = "Percent of renters\nutilizing HCV",
         caption = "**Source:** U.S. Department of Housing and Urban Development.<br>**Note:** Tracts with no renters using HCVs are transparent.") +
    theme_hfv() +
    theme(
      legend.position = "right",
      legend.title = element_text(),
...
```{r}
#| label: fig-local-cb
#| fig-cap: "Cost-burdened households by tenure in Lynchburg"
...
cv_cb_plot <- ggplot(local_cb,
  ...
  scale_fill_hfv() +
  scale_y_continuous(labels = label_number(big.mark = ",")) + 
  labs(title = "Cost-burdened households by tenure in Lynchburg",
       subtitle = "Number of cost-burdened <span style = 'color:#011E41'><b>homeowners</b></span> and <span style = 'color:#40C0C0'><b>renters</b></span> from 2010 to 2021",
...
```{r}
#| label: fig-cb-race
#| fig-cap: "Cost burden by race and ethnicity in Lynchburg"
...
cv_cb9_plot <- ggplot(cv_cb9,
  ...
  facet_wrap(~race, scales = "free_x", labeller = label_wrap_gen(25)) +
  scale_y_continuous(labels = percent_format()) +
  scale_fill_hfv() +
  theme_hfv() +
  add_zero_line("y") +
  ...
  labs(title = "Cost burden by race and ethnicity in Lynchburg",
       subtitle = "Percent of cost-burdened households from 2012 to 2020",
