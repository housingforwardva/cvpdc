...
# Mortgage delinquency from CPFB (low priority, but in our proposal) - this isn't available for the region or localities in the region
...
# Assess errors for Fig 5.22 - remove all but Black and White households - MOE is high
...
write_rds(qcew_data, "data/qcew_data.rds")
```
...
cv_qcew <- read_rds("data/qcew_data.rds") |> 
  ...
  left_join(cv_names, by = "GEOID") |> 
  select(county, year, emplvl = annual_avg_emplvl, wkwage = annual_avg_wkly_wage, salary = avg_annual_pay)
...
# Need to adjust to 2021 dollars.
...
ggplot(cv_qcew,
  ...
  scale_color_hfv() +
  facet_wrap(~county) +
  labs(title = "Annual average weekly wage levels",
       caption = "**Source:** U.S. Bureau of Labor Statistics, Quarterly Census of Employment and Wages (QCEW).") +
  scale_y_continuous(labels = number_format(big.mark = ","))
...
The margins of error for Asian and Hispanic households, as well as other minority groups in the region (multiracial households included), are significantly high. This is most often due to the small sample sizes.
