...
```{r}
#| label: fig-localpop
#| fig-cap: "2020 Census population by county"
...
```{r}
#| label: fig-localproj
#| fig-cap: "Projected population growth 2020 to 2050 by county"
...
local_proj_plot <- ggplot(pop_proj,
  ...
  labs(title = "Projected growth from 2020 to 2050",
...
#| fig-cap: "Local cumulative change in households by tenure"
...
```{r}
#| label: fig-localage
#| fig-cap: "Local percent change in households by age and tenure"
...
```{r}
#| label: fig-localsize
#| fig-cap: "Local cumulative change in households by size and tenure"
...
#| fig-cap: "Local change in senior population living alone"
...
local_senior_plot <- ggplot(local_senior,
  ...
  labs(title = "Change in senior population living alone",
       ...
       caption = "**Source:** U.S. Census Bureau, ACS 5-Year Estimates, Table B09020.")
...
#| fig-cap: "Local income distribution by tenure"
...
local_dist_plot <- ggplot(local_dist,
  ...
  scale_y_continuous(labels = label_comma()) +
  labs(title = "Income distribution in 2021",
...
```{r}
#| label: fig-med-inc
#| fig-cap: "Local median household income by tenure"
...
#| fig-cap: "Local median household income by race and ethnicity"
...
The counties contribute to 36 percent of the region's total housing stock. The majority of that share consists of single-family housing that is utilized for homeownership. Among the homeownership housing stock in the counties, there is little diversity as the only other type of housing is in the "Other" category, which includes manufactured homes. Two to four unit homes, like duplexes and quads, can look like single-family housing and can often be more affordable, but the counties only contribute one percent of this type of housing to the region's total stock.

Renter housing stock in the counties is diverse, but still largely made up of single-family housing. 

```{r}
#| label: fig-localstructure
#| fig-cap: "Distribution of house stock in the region"
...
#| fig-cap: "Local manufactured home communities by size"
...
```{r}
#| label: fig-localbps
#| fig-cap: "Local esidential building permits by type"
...
cv_cbps_plot <- ggplot(cv_cbps,
  ...
  theme_hfv() +
  scale_fill_hfv() +
  labs(title = "Residential building permits by type",
...
```{r}
#| label: fig-localho
#| fig-cap: "Local homeownership rates"
...
From 2015 to 2020, rental housing costs in the counties have remained relatively flat. But as rental prices in Bedford began to rise gradually, prices in Amherst, Appomattox, and Campbell saw declines.

:::{.callout-note}

Add blurb about small sample sizes for Amherst and Appomattox.
...
```{r}
#| label: fig-localrent
#| fig-cap: "Local quarterly average market asking rents"
...
local_rent_plot <- ggplot(local_rent,
  ...
  labs(title = "Quarterly average rent",
       subtitle = "Market asking rent from 2015 through 2022",
...
```{r}
#| label: fig-nhpd
#| fig-cap: "Federally assisted rental homes by county"
...
```{r}
#| label: fig-hcv-map
#| fig-cap: "Housing Choice Voucher utilization in the counties"
...
hcv_grp_plot <- ggplot(hcv_group,
  ...
  theme_hfv() +
  scale_fill_hfv() +
  labs(title = "Housing Choice Voucher utilization in the counties",
...
cv_cb_plot <- ggplot(local_cb,
  ...
  labs(title = "Local cost-burdened households by tenure",
...
```{r}
#| label: fig-cb-race
#| fig-cap: "Local cost burden by race and ethnicity"
...
cv_cb9_plot <- ggplot(cv_cb9,
  ...
  labs(title = "Cost burden by race and ethnicity",
       subtitle = "Percent of cost-burdened households in 2019",
...
```{r}
#| label: fig-hh-cb
#| fig-cap: "Local cost-burden by household type"
...
cb_hh_plot <- ggplot(cbhh_region,
  ...
  theme(axis.text.x = element_blank(),
    ...
    facet_wrap(~county, nrow = 1, labeller = label_wrap_gen(10)) +
  scale_y_continuous(labels = percent_format()) +
  labs(title = "Local cost-burden by household type",
...
```{r}
#| label: fig-gap
#| fig-cap: "Local rental housing gap by AMI"
...
gap_plot <- ggplot(gap,
  ...
  geom_col() +
  geom_col_interactive() +
  facet_grid(household_income~county, labeller = label_wrap_gen(10)) +
  scale_y_continuous(labels = label_comma()) +
  theme_hfv() +
  scale_fill_hfv() +
  labs(title = "Local rental housing gap by AMI",
